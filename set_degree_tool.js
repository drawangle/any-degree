...
function drawangle_degree(any_degree = Number(document.getElementById("anydegree").value)) {
    ...
    knob.setAttribute('transform', `rotate(${ 0 - angle})`);
    ...
    */
    ...
    // SVG 再作成
    document.getElementById("current-angle").textContent = Math.trunc(finalAngle);
    degree = Math.trunc(finalAngle);
    updateGallery();

    // Macyに再計算させる
    // 画像の読み込み完了後に自動で再計算を実行
    macy.runOnImageLoad(function () {
        macy.recalculate();
    }, true); // 第2引数を true にすると、読み込み完了時だけでなく即時実行も試みます
...
function drawangleSVG(angle, finalAngle) {
    //console.log('drawangleSVG',angle, finalAngle)
    ...
    dSegments.push(`A ${_r} ${_r}  0 ${Number(angle > 180 || angle < -180)} ${Number(finalAngle < 0)}  ${ex} ${ey}`);// angle が正なら0、負なら1
...
function updateAngle(event) {
    ...
    // 5. SVGのツマミを回転させ、テキストを更新
    knob.setAttribute('transform', `rotate(${ 0 - angle}, ${centerX}, ${centerY})`);
    angle_display.textContent = `${Math.trunc(Math.abs(finalAngle))}°`;
    old_value = document.getElementById("anydegree").value;
    ...
    */
    ...
    // spiralの描画
    if (old_value != document.getElementById("anydegree").value) {
        //console.log('updateAngle', old_value, angle, degree);
        //num = Number(document.getElementById('current-angle').textContent);
        document.getElementById("current-angle").textContent = Math.trunc(finalAngle);
        degree = Math.trunc(finalAngle);
        //console.log('updateAngle', old_value, angle, degree);
